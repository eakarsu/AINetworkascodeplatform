// Auto-generated feature route: capacity-planning-ai
// Domain: capacity planning AI — Capability: predicting bandwidth demand 6-12 months out
// Mounted under: /api/capacity-planning-ai
...
// POST /api/capacity-planning-ai/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a capacity planning AI. Your task is predicting bandwidth demand 6-12 months out. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/capacity-planning-ai/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/capacity-planning-ai/:id — fetch one
...
// DELETE /api/capacity-planning-ai/:id — discard a run
