...
// Route: /api/gap-oss-bss
// Feature: Oss Bss
// Description: OSS/BSS integration (Amdocs, Openet) webhooks
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Oss Bss. OSS/BSS integration (Amdocs, Openet) webhooks' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'oss-bss', title: 'Oss Bss', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'oss-bss', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['oss-bss']);
      return res.json({ slug: 'oss-bss', items: r.rows || [] });
    }
    return res.json({ slug: 'oss-bss', items: [] });
  } catch (e) {
    return res.json({ slug: 'oss-bss', items: [], error: (e && e.message) || 'Server error' });
  }
});
