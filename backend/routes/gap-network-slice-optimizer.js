...
// Route: /api/gap-network-slice-optimizer
// Feature: Network Slice Optimizer
// Description: (slice allocation optimization)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Network Slice Optimizer. (slice allocation optimization)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Network Slice Optimizer\nDescription: (slice allocation optimization)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['network-slice-optimizer', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'network-slice-optimizer', title: 'Network Slice Optimizer', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'network-slice-optimizer', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['network-slice-optimizer']);
      return res.json({ slug: 'network-slice-optimizer', items: r.rows || [] });
    }
    return res.json({ slug: 'network-slice-optimizer', items: [] });
  } catch (e) {
    return res.json({ slug: 'network-slice-optimizer', items: [], error: (e && e.message) || 'Server error' });
  }
});
