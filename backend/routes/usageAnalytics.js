...
const page = Math.max(1, parseInt(req.query.page) || 1);
    ...
    const countResult = await pool.query('SELECT COUNT(*) FROM usage_analytics');
    const total = parseInt(countResult.rows[0].count);
    const result = await pool.query('SELECT * FROM usage_analytics ORDER BY created_at DESC LIMIT $1 OFFSET $2', [limit, offset]);
