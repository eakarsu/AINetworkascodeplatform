// Auto-generated feature route: threat-detection-agent
// Domain: real-time threat detection agent — Capability: suggesting auto-mitigation steps
// Mounted under: /api/threat-detection-agent
...
// POST /api/threat-detection-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a real-time threat detection agent. Your task is suggesting auto-mitigation steps. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/threat-detection-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/threat-detection-agent/:id — fetch one
...
// DELETE /api/threat-detection-agent/:id — discard a run
