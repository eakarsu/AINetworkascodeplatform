...
// Route: /api/gap-multi-operator
// Feature: Multi Operator
// Description: Multi-operator federation (inter-carrier coordination)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Multi Operator. Multi-operator federation (inter-carrier coordination)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'multi-operator', title: 'Multi Operator', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'multi-operator', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['multi-operator']);
      return res.json({ slug: 'multi-operator', items: r.rows || [] });
    }
    return res.json({ slug: 'multi-operator', items: [] });
  } catch (e) {
    return res.json({ slug: 'multi-operator', items: [], error: (e && e.message) || 'Server error' });
  }
});
